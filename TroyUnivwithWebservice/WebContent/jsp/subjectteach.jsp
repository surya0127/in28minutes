...
.back {
    background-image: url("./../images/stu5.jpg");
	width:100%;
	height:100%;
   color:black;
   }
...
function loadsubjects() {
	...
	$("#table1 tbody").find('input[name="chkboxName"]').each(function(){
        ...
        $(this).parents("tr").remove();
			...
			$.ajax({
			type : 'POST',
			url : "./getStaffSubjectReg.htm",
			dataType : 'json',
			data : {'dept':dept,'sem' :sem },
			success : function(data) {
				$.each(data.subjectdetails, function(i, item) {
					var markup = "<tr><td>" + item.subjectid +"</td><td>" + item.subjectname +  "</td><td>" +item.timings+"</td><td>" +item.Officehours +"</td><td>"+item.seats+"<td><input type='checkbox' name='chkboxName' id ='"+item.subjectid+"' value='"+item.subjectid+"'></td></tr>";
...
$.ajax({
	type : 'POST',
	url : "./getSubjectRegView.htm",
	dataType : 'json',
	success : function(data) {
		$.each(data.subjectdetails, function(i, item) {
			var markup = "<tr><td>" + item.subjectid +"</td><td>" + item.subjectname +  "</td><td>" +item.timings+"</td><td>" +item.Officehours +"</td><td>"+item.seats+"</td></tr>";
		...
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav"
				...
				<li><a href="redirect.htm?id=subjectteach"
					style="background-color: #adc4ea"><b>View Notifications</b></a></li>
