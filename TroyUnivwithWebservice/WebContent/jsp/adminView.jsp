...
function loadAccesApprovals() {
	...
	$("table tbody").find('input[name="chkboxName"]').each(function(){
        ...
        $(this).parents("tr").remove();
			...
			$.ajax({
			type : 'POST',
			url : "./getapproveAccessList.htm",
			dataType : 'json',
			success : function(data) {
				$.each(data.approveDetails, function(i, item) {
					var markup;
					 markup = "<tr><td>" + item.NAME + "</td><td>" + item.USER_NAME + "</td><td>" +item.USER_TYPE+"</td><td><input type='checkbox' style='padding: 1px 1px;width: 55px;' apprAttr='approve' name="+item.id+" id="+item.id+"></td><td><input type='checkbox' style='padding: 1px 1px;width: 55px;' apprAttr='reject' name="+item.id+" id="+item.id+"></td></tr>";	
...
$(document).on('click','#questionSub',function(){
	...
	var rejectList=[];
	        $("input:checkbox").each(function(){
	        	 var $this = $(this);
		        if($this.is(":checked")){
		        	console.log($this.attr("apprAttr"));
		        	if($this.attr("apprAttr") == "approve"){
		        		accessList.push($this.attr("id"));
		        	} else if($this.attr("apprAttr") == "reject"){
		        		rejectList.push($this.attr("id"));
		        	}
	...
	if(accessList.length > 0){
		$.ajax({
			type : 'POST',
			url : "./saveAccess.htm?accessList="+accessList,
			dataType : 'json',
			success: function(data){
				console.log(data);
	...
	if(rejectList.length > 0){
		$.ajax({
			type : 'POST',
			url : "./rejectAccess.htm?rejectList="+rejectList,
			dataType : 'json',
			success: function(data){
				console.log(data);
				if(data.status == 'success')
...
<body>
	<nav class="navbar navbar-fixed">
		<div class="container-fluid">
			...
			<li><a href="redirect.htm?id=adminView"	style="background-color: #adc4ea"><b>Admin View</b></a></li>
			<li><a href="redirect.htm?id=adminsemisterView"><b>Update Semester</b></a></li>
