...
function showSelectedValues()
{
	...
	var msg = $('textarea#resolution').val();
	//alert('msg'+msg);
	
	$.ajax({
		type : 'POST',
		url : "./saveresolution.htm",
		dataType : 'json',
		data : {'selctedsub':selsubjects,'msg':msg },
		success : function(data) {
	...
	$.ajax({
		...
		success : function(data) {
			$.each(data.notificationdetails, function(i, item) {
				var markup = "<tr><td>" + item.issueid + "</td><td>" + item.issuemsg + "</td><td>" + item.departmentname + "</td><td>" +item.issueopendate+ "</td><td>" +item.resolution+ "</td><td>" + item.closedate + "</td><td><input type='checkbox' name='chkboxName' id ='"+item.issueid+"' value='"+item.issueid+"'></td></tr>";
	            $("table tbody").append(markup);
			});
		},
		failure : function() {
...
<body>
	<nav class="navbar navbar-fixed">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav"
				...
				<li><a href="redirect.htm?id=subjectteach"
					><b>View Notifications</b></a></li>
					<li><a href="redirect.htm?id=mysubject"  ><b>Issue Mgmt</b></a></li>
				<li><a href="redirect.htm?id=staffsubDrop" style="background-color: #adc4ea" ><b>Issue Resolution
							</b></a></li>
	...
	<div class="back">
		...
		<div align="center">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<textarea 
				rows="4" cols="50"  placeholder="Enter Resolution  Message" name="resolution" id ="resolution" required></textarea ><br>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<div align="center">
			<button type="submit" style="padding-left: 50px"
				onclick="showSelectedValues()">Submit Resolution</button>
		</div>
	</div>
	<br>
	<br>
