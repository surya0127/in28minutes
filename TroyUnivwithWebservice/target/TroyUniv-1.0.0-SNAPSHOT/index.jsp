		...
		console.log(unamein);
			...
			success: function(data){
				if(data){
					...
					alert('Answers are mismatching,Please correct the answers');
					return false;					
				}
			},
			failure : function() {
...
function signIn() {
	...
	$.ajax({
		type : 'POST',
		url : "./CIS/userRegistration.htm",
		data : { 'uname' : name, 'psw':psw},
		dataType : 'json',
		success: function(data){
			if(data.status == 'A'){
				window.location.href ="./CIS/redirect.htm?id=adminlogin"; }
			else if(data.status == 'S'){
				window.location.href ="./CIS/redirect.htm?id=studentlogin"; }
			else if(data.status == 'F'){
				window.location.href ="./CIS/redirect.htm?id=stafflogin"; }
			else if(data.status == '1'){
				alert('Your request not yet approve by Admin,Please contact Admin!!!!');
				$('#unamein').val("");
				 $('#pswin').val("");
			}else {
				alert(' Invalid Username and Password, Please try arain!!!!');
				$('#unamein').val("");
				 $('#pswin').val("");}
		},
		failure : function() {
	...
	<nav class="navbar">
		<div class="container-fluid">
			...
			<div class="navbar-header"
				...
				<h1
					style="font-family: Broadway; color: black; text-align: center; padding-top: 15px">
					<b>ESAIP Graduate School of Engineering</b>
				</h1>
